...
namespace Gorge.GorgeCompiler.Expression.AssignmentTarget
{
    ...
    /// 表达式返回类型为待赋值字段的类型
    /// </summary>
    public class InjectorFieldAccess : AssignmentTargetExpression
    {
        ...
        public InjectorFieldAccess(AssignmentTargetExpression objectOperand, InjectorFieldSymbol field,
            CodeBlockScope block, ParserRuleContext antlrContext) : base(block, antlrContext)
        {
            _objectOperand = objectOperand;
            FieldIndex = field.Index;
            AssignmentTargetType = AssignmentTargetType.InjectorField;
            ValueType = objectOperand.AssignType;
            AssignType = field.FieldType;
        }
